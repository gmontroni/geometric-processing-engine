...
def compute_surface_operators3dd(pts, T, B, N, angle_threshold_degrees=150):
    nopts = pts.shape[0]
    tree = KDTree(pts)

    dotProduct = np.dot(N, N.T)
    cosTheta = np.clip(dotProduct, -1.0, 1.0)
    ...
    for i in range(nopts):
        ...
        # Critério 1: ângulo entre normais
        cos_angles = cosTheta[i, idx]
        mask = cos_angles > angle_threshold_cos
        filtered_idx = idx[mask]

        # fallback se poucos vizinhos
        if filtered_idx.size < 35:
            filtered_idx = idx[:35]

        # Critério 2: razão d_perp / d_parallel
        n_p = N[i]
        diffs = pts[filtered_idx] - pts[i]  # shape: (n, 3)

        d_perp = np.abs(np.dot(diffs, n_p))  # projeção na normal (altura fora do plano)
        diffs_proj = diffs - np.outer(d_perp, n_p)  # projeção no plano
        d_parallel = np.linalg.norm(diffs_proj, axis=1)

        score_proj = d_perp / (d_parallel + eps)
        sorted_indices = np.argsort(score_proj)
        final_idx = filtered_idx[sorted_indices[:25]]  # pega os 20 com menor score

        # Operadores finais
        R = np.hstack((T[i,:].reshape(3,1), B[i,:].reshape(3,1)))
        Xloc = R.T @ (pts[i, :] - pts[final_idx, :]).T
        Xloc = Xloc.T
        ...
        temp = R[:, :2] @ W[:, 1:3].T
        Gx3D[i, final_idx] = temp[0, :]
        Gy3D[i, final_idx] = temp[1, :]
        Gz3D[i, final_idx] = temp[2, :]
